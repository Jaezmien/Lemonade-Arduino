...
def on_buffer_read(buffer):
	data = pad_as_bytes(buffer[0], buffer[1], buffer[2], buffer[3])
...
def on_tick():
	...
	# global NotITG
	# left = NotITG.GetExternal(200)
	# down = NotITG.GetExternal(201)
	# up = NotITG.GetExternal(202)
	# right = NotITG.GetExternal(203)
	
	# print(f"{left}{down}{up}{right}")
	
	# data = pad_as_bytes(left, down, up, right)
	# if data is not last_seen_data:
	# 	arduino.write(data)
	# 	last_seen_data = data
	pass
